$(document).ready(function(){
    $("#save").click(function(){
        ...
        var nick = $("#nick").val();
        var portrait = $("#portrait").attr("src").split("?")[0];
         if( !nick){
...
var uploadPortrait = (function(){
    ...
    var upload = function(portrait,callback){
        var c = common.warns("正在上传...","c-warn_process");
        common.removeWarn(c, 5000);
        $.ajax({
            "url" : url,
            "type" : "post",
            "timeout" : 5000,
            ...
            "success" : function(data){
                callback(data);
                var s = common.warns("上传成功！","c-warn_tips");
                common.removeWarn(s, 5000);
            },
            "error" : function(err){
                var f = common.warns("上传失败！","c-warn_fail");
                common.removeWarn(f, 5000);
            }
        });
    };
    return {
...
window.addEventListener("message",function(e){
    ...
    if(temp.from === "cropper"){
        uploadPortrait.upload(temp.data,function(data){
            $("#save").removeClass("g-distabled");
            $("#portrait").attr("src", data.url+"?imageView2/1/w/200/h/200").css({"display":"block"});
            $("#cropper").hide();
        });
    }
},false);